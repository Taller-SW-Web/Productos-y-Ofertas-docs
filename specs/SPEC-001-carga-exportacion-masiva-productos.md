...
La plantilla posee columna `operacion=CREAR_PRODUCTO_SIMPLE | CREAR_VARIANTE | ACTUALIZAR` y columnas obligatorias de identificación según operación. `CREAR_PRODUCTO_SIMPLE` requiere `sku_base` nuevo y `tiene_variantes=false`. `CREAR_VARIANTE` requiere `product_id` o `sku_base` de un padre `tiene_variantes=true` ya existente **o declarado para creación en el mismo lote**, `tipo_producto_id`, atributos identificadores y un **SKU comercial opcional**: si viene vacío, Catálogo lo genera; si viene informado, valida formato y unicidad global antes de aceptarlo. El identificador interno de la variante (`variant_id`) siempre es generado por Catálogo y no depende del SKU comercial. Si el padre aún no existe, la primera fila del grupo aporta sus metadatos obligatorios y precio base, y todas las filas con ese `sku_base` deben coincidir en esos datos; Catálogo crea un único padre BORRADOR idempotente antes de sus variantes. Una discrepancia entre filas del mismo padre rechaza el grupo afectado. Catálogo devuelve `product_id`, `variant_id`, `sku` y `batch_id`/`row_id`. `ACTUALIZAR` requiere SKU existente y prohíbe modificar `sku_base`, `tiene_variantes`, `tipo_producto_id` cuando ya forma parte de una identidad publicada y los atributos identificadores de una variante existente. La plantilla usará las cabeceras, orden y tipos del Requisito 9, con `template_version=2`, sin mapeo dinámico. Si el catálogo exporta variantes en borrador, su estado se incluye y **no se activa implícitamente** al importarlas.
...
- ENTONCES crea un único padre BORRADOR y dos variantes BORRADOR, asigna un `variant_id` interno a cada una, valida el SKU comercial aportado o genera uno cuando se dejó vacío, y coordina sus preparaciones sin vender artículos incompletos.
...
### Requisito 7: Control de concurrencia por dominio y ajuste absoluto de stock seguro
La exportación incluye versiones esperadas de los datos que pueden modificarse fuera del archivo: `catalog_version`, `price_version` y `stock_version`. En `ACTUALIZAR`, si una fila modifica campos de Catálogo o Pricing debe conservar la versión exportada correspondiente; el dominio propietario rechaza con `VERSION_CONFLICT` cuando la entidad fue modificada después de la exportación, evitando que un archivo antiguo sobrescriba silenciosamente cambios más recientes. El gestor debe reexportar o revisar el conflicto antes de reenviar; no se reintenta automáticamente una escritura absoluta obsoleta.

Una celda de stock no vacía expresa **conteo absoluto** del SKU en `location_id`. Para `ACTUALIZAR` requiere `stock_version` de la exportación o versión esperada explícita. Para un SKU recién creado, Bulk espera la confirmación de inicialización en Inventario con saldo 0 y utiliza la versión inicial `0`; no solicita ajustar un SKU aún inexistente. Inventario ajusta mediante comparación atómica de versión y registra en Kardex valor anterior, nuevo, delta y motivo `BULK_IMPORT`. Si una venta o ajuste modificó el saldo/versión, rechaza el cambio con `VERSION_CONFLICT`. Una celda de stock vacía mantiene el saldo sin ajuste. La actualización condicional usa bloqueos de fila transaccionales breves por `(sku, location_id)`, sin bloqueos globales del inventario ni garantía irreal de ausencia absoluta de espera.
...
### Requisito 9: Contrato de plantilla general v2 y exportación asíncrona
Las columnas **en este orden exacto** para CSV/XLSX v2 son: `operacion`, `product_id`, `variant_id`, `sku_base`, `sku`, `nombre`, `descripcion`, `categoria_id`, `tipo_producto_id`, `marca_id`, `tiene_variantes`, `caracteristicas_identificadoras`, `atributos_identificadores`, `atributos_no_identificadores`, `imagen_url`, `precio_regular`, `precio_oferta`, `accion_precio_oferta`, `location_id`, `stock`, `catalog_version`, `price_version`, `stock_version`, `estado`, `motivo_cambio`. Todas las cabeceras deben existir, aunque algunas celdas sean opcionales según `operacion`; un cambio del esquema requiere nueva `template_version`.

La plantilla mantiene identificadores estables para integridad, pero **no exige que el gestor redacte JSON manualmente en XLSX**. La versión XLSX incluye hojas auxiliares protegidas, catálogos de referencia y validaciones/listas para seleccionar características y valores; el archivo generado transforma esas selecciones a la representación técnica antes de enviarla. En CSV, las celdas de atributos admiten la representación compacta documentada para integraciones o usuarios avanzados. El backend sigue validando IDs y nunca confía en texto visible como identidad. De esta forma se conserva un contrato versionado y estable sin convertir la edición ofimática ordinaria en edición manual de estructuras técnicas.

Identificadores y versiones son strings/enteros según su entidad; `tiene_variantes` usa `true|false`; importes usan decimales con punto y dos fracciones para PEN; `stock`, `catalog_version`, `price_version` y `stock_version` son enteros >=0. `location_id` identifica la ubicación de inventario; para el MVP puede emplearse la ubicación predeterminada configurada por Inventario. Las columnas `caracteristicas_identificadoras` y `atributos_identificadores` conservan la representación técnica estable generada por la plantilla XLSX o suministrada por una integración CSV. `atributos_no_identificadores` es opcional. No se ejecutan fórmulas.

- `CREAR_PRODUCTO_SIMPLE`: `sku_base`, `nombre`, `descripcion`, `categoria_id`, `tipo_producto_id`, `marca_id`, `tiene_variantes=false`, `precio_regular` y `motivo_cambio` son obligatorios; `sku` puede estar vacío o igual a `sku_base`; no se declaran características identificadoras.
- `CREAR_VARIANTE`: `sku_base`, `tipo_producto_id`, `tiene_variantes=true`, características/valores identificadores e `imagen_url` son obligatorios; `sku` es opcional: vacío implica generación automática y un valor informado se trata como SKU comercial solicitado y se valida por unicidad. `variant_id` siempre queda vacío en creación porque lo genera Catálogo. Un padre nuevo exige además en **cada fila de su grupo** `nombre`, `descripcion`, `categoria_id`, `marca_id`, `precio_regular` y `motivo_cambio` consistentes. Para un padre existente se identifica por `product_id` o `sku_base` y se comprueba que su tipo de producto y sus características identificadoras coincidan con las ya fijadas.
- `ACTUALIZAR`: `sku` existente es obligatorio; `variant_id` puede viajar como referencia adicional. Las celdas vacías conservan datos. Si se modifican campos de Catálogo se exige `catalog_version`; si se modifica precio, `price_version` y `motivo_cambio`; si se informa `stock`, se exige `location_id` y `stock_version`. `sku_base`, `tiene_variantes`, la identidad de tipo publicada y los atributos identificadores no se cambian mediante este flujo ordinario.
